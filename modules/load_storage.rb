  ...
  def create_person_from_data(data_base)
    ...
    data_base.each do |data|
      if data['type'] == 'Student'
        ...
        person = Student.new(classroom, data['age'], parent_permission: data['parent_permission'], name: data['name'],
                                                     id: data['id'])
        person.type = data['type']
      elsif data['type'] == 'Teacher'
        person = Teacher.new(data['specialization'], data['age'], name: data['name'], id: data['id'])
        person.type = data['type']
      end
      people << person
    end
